...
#     global magneet_waarde,poweroff
#     time.sleep(0.05)
#     magneet_waarde = GPIO.input(poweroff)
#     # print(f'poweroff>{magneet_waarde}')
#     # if magneet_waarde == 0:
#     #     magneet_nok()
#     # elif magneet_waarde == 1:
#     #     magneet_ok()

GPIO.add_event_detect(reed_contact, GPIO.BOTH,
                      callback=callback_reedcontact, bouncetime=200)
# GPIO.add_event_detect(poweroff, GPIO.RISING,
#                       callback=callback_reedcontact1, bouncetime=200)
...
def display_id(shared_data, shared_user,shared_servo,gewicht_value):
...
def check_process_data():
    global rfid_data, socketio, servo, rfid_user, gewicht, start_time
    start_time = None
    while True:
...
def multiprocess_display_ip():
    p1 = multiprocessing.Process(
        target=display_id, args=(rfid_data, rfid_user,servo_value,gewicht_value))
    p1.start()
...
def data_gewichtsensor_offset():
    ...
    while True:
        ...
        correct_format_date = now.strftime("%d/%m/%Y %H:%M:%S")
        if -5 < gewicht < 2:
            socketio.emit('leegpost', {'waarde': 'leeg'}, broadcast=True)
            user_id = int(rfid_user.value)
            if user_id != 0:
            # print(user_id)
            ...
            socketio.emit('post', {'waarde': 'aanwezig'}, broadcast=True)
        # time.sleep(1)
...
def initial_connection():
    ...
    emit('ipadres', {'adres': ip_adres[1]}, broadcast=True)
...
# @socketio.on('F2B_RFID_read')
# def switch_servo(payload):
#     print(payload)
#     while True:
#         id_rfid, text = reader.read_no_block()
#     # global servo, socketio
#     # socketio.emit("B2F_close", {'Deur': 'toe'}, broadcast=True)
#     # servo.sluit_deur()
